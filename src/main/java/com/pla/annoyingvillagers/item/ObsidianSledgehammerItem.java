...
public class ObsidianSledgehammerItem extends LegacySwordItem implements RigCombatProfileProvider {
    ...
    public ObsidianSledgehammerItem() {
        ...
        }, 1, -2.6F, (com.pla.annoyingvillagers.util.LegacyItemProperties.create()).fireResistant());
    }
    ...
    public static boolean activateVanillaSpecial(Player player) {
        ...
        if (!(stack.getItem() instanceof ObsidianSledgehammerItem item) || player.getCooldowns().isOnCooldown(new net.minecraft.world.item.ItemStack(item))) return false;
        VanillaWeaponAbilityUtil.swingMainHand(player, VanillaWeaponAbilityUtil.BETTER_COMBAT_TWO_HANDED_SLAM);
        ...
        player.getCooldowns().addCooldown(new net.minecraft.world.item.ItemStack(item), 20 * 60);
        return true;
    }
    ...
    public static void spawnWave(LivingEntity caster, float yaw, float innerRadius, float radius, int particleCount) {
        ...
        for (int i = 0; i < particleCount; i++) {
            ...
            float particlePitch = -30.0F + level.getRandom().nextFloat() * 60.0F - 30.0F;
...
//        }
    ...
    public void appendHoverText(@NotNull ItemStack itemstack, net.minecraft.world.item.Item.TooltipContext level, @NotNull net.minecraft.world.item.component.TooltipDisplay display, java.util.function.Consumer<Component> list, @NotNull TooltipFlag tooltipflag) {
        ...
        list.accept(Component.translatable("tooltip.annoyingvillagers.obsidian_sledgehammer"));
    }
