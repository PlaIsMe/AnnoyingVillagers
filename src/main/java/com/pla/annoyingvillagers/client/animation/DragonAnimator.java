...
//        jawTimer.add(jawFlag? 0.2f : -0.2f);
    ...
    protected void animHeadAndNeck(ModelHerobrineDragon model)
    {
        ...
        for (int i = 0; i < model.neckProxy.length; i++)
        {
            ...
            neckSize = ModelHerobrineDragon.NECK_SIZE * ((ModelPartAccess) (Object) model.neck).getZScale() - 1.4f;
...
//        model.wingArm.xRot += 1 - speed;
    ...
    protected void animTail(ModelHerobrineDragon model)
    {
        ...
        for (int i = 0; i < model.tailProxy.length; i++)
        {
            ...
            model.tail.xRot += (float) Math.toRadians(pitchOfs);
            model.tail.xRot -= (1 - speed) * vertMulti * 2;
            model.tail.yRot += (float) Math.toRadians(180 - yawOfs);
            ...
            // move next proxy behind the current one
            float tailSize = ModelHerobrineDragon.TAIL_SIZE * ((ModelPartAccess) (Object) model.tail).getZScale() - 0.7f;
