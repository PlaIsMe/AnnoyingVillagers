...
public final class RecoveryAi {
    ...
    public static boolean canStart(AVNpc npc) {
        String reason = !canAct(npc) ? "cannot_act_see_flags"
                ...
                : NeoForge.EVENT_BUS.post(new AVNpcRecoveryEvent(npc, AVNpcRecoveryEvent.Action.CHECK_START)).isCanceled()
    ...
    public static void startRecovery(AVNpc npc) {
        ...
        NeoForge.EVENT_BUS.post(new AVNpcRecoveryEvent(npc, AVNpcRecoveryEvent.Action.START));
    }
    ...
    public boolean breakBlock(BlockPos pos, boolean bodyOverlap) {
        ...
        if (breakTicks % 10 == 0 && !NeoForge.EVENT_BUS.post(new AVNpcRecoveryEvent(npc, AVNpcRecoveryEvent.Action.DIG)).isCanceled()) {
        ...
        if (level.getBlockState(pos) == state && canBreak(npc, pos)
                && EventHooks.onEntityDestroyBlock(npc, pos, state)) {
    ...
    public void pauseBreaking() {
        ...
        if (npc.isRecoveryDigging()) {
            ...
            NeoForge.EVENT_BUS.post(new AVNpcRecoveryEvent(npc, AVNpcRecoveryEvent.Action.STOP_DIG));
        }
    }
    ...
    public boolean placeUnderFeet(BlockPos pos) {
        ...
        if (EventHooks.onBlockPlace(npc, snapshot, Direction.UP)) {
            snapshot.restore(3);
        ...
        if (!NeoForge.EVENT_BUS.post(new AVNpcRecoveryEvent(npc, AVNpcRecoveryEvent.Action.USE)).isCanceled()) {
