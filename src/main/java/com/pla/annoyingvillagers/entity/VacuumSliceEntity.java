...
public class VacuumSliceEntity extends Entity {
    ...
    private Vec3 spawnHorizontalImpact(ServerLevel level, Vec3 impactPosition, Vec3 velocity) {
        ...
        for (int i = -halfSamples; i <= halfSamples; i++) {
            ...
            LevelUtil.circleSlamFracture(null, level, fracturePosition(hit), FRACTURE_RADIUS, i != 0, false, false);
            level.sendParticles(new SmokeWaveOptions(cutYaw, 0.0D, (i & 1) == 0 ? 0.0D : 180.0D), surface.x, surface.y + PARTICLE_Y_OFFSET, surface.z, 1, 0.0D, 0.0D, 0.0D, 0.0D);
        }
    ...
    private void spawnGroundSlash(ServerLevel level, Vec3 position, float yaw, double roll) {
        ...
        if (hit == null) return;
        Vec3 surface = surfacePosition(hit);
        level.sendParticles(new GroundSlamOptions(yaw, 0.0D, roll), surface.x, surface.y + PARTICLE_Y_OFFSET, surface.z, 1, 0.0D, 0.0D, 0.0D, 0.0D);
    }
    ...
    private BlockHitResult findGround(ServerLevel level, Vec3 position) {
        ...
        return LevelUtil.canTransferShockWave(level, hit.getBlockPos(), state) ? hit : null;
    }
